...
//----------------------------------------------------------------------------------------------------------------//
	...
	sub_block_cycles = (isr_time_cycles % (int) INST_UNIT == 0) ? isr_time_cycles : (isr_time_cycles % (int) INST_UNIT) + isr_time_cycles;	
	rem_cycles_temp = sub_block_cycles - rem_block_cycles; 
	if(rem_cycles_temp > 0) {
	// Crossing multiple basic blocks within one period of interrupt timer
	...
	else if(rem_cycles_temp == 0) {
	// Remaining workload of current basic block takes same time as one period of interrupt timer
	...
	// Remaingin workload of current basic block takes a greater number of required exeuction time than one period of interrupt timer
