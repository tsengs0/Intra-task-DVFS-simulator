...
void Task_Scheduler::sched_arbitration(float sched_tick)
{
	...
	#ifdef DEBUG
		cout << "Task_" << running_task_id << " complete at " << time_management -> sys_clk -> cur_time << " s" << endl;
	#endif
		running_task_id = (int) CPU_IDLE;
	}
	...
	#ifdef DEBUG
		 cout << "RWCEC for Task_" << running_task_id << ": " << rwcet << "(" << time_management -> sys_clk -> cur_time << ")" << endl;
	#endif
	}
#else
	...
	for(int i = 0; i < task_list.size(); i++) {
		if(task_list[i].release_time == /*time_management -> sys_clk -> cur_time*/sched_tick) { //
...
void Task_Scheduler::dispatcher(void)
{
	...
	if(pre_task != running_task_id && pre_task != (int) CPU_IDLE) {
		cout << "preemption(pre:" << pre_task << ", new:" << running_task_id << ")" << endl;
		context_switch(pre_task, running_task_id);
	} // Preemption
	else if(isr_stack.isr_flag == true && isr_stack.task_id == running_task_id) {
		cout << "resume" << endl; 
		resume();
	} // Resuming from previous preemtion
	else {
